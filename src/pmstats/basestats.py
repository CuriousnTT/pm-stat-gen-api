...
""" class PmStats(Base):
    ...
    pm_stats_nat_dex_nr: Mapped[int] = mapped_column(Integer, ForeignKey('form.nat_dex_nr'), primary_key=True)
...
""" class PmStatValue(Base):
    ...
    pm_stats_nat_dex_nr: Mapped[int] = mapped_column(Integer, ForeignKey('pm_basestats.pm_stats_nat_dex_nr'), primary_key=True)
    ...
    stat: Mapped[Stat] = relationship('Stat', foreign_keys=[stat_id])
...
def get_generic_base_stats(gen: Union[PmGen, None] = None):
    ...
    base_stats = [stat for stat in stats if stat not in absent_stats]
    baseStatsInfo = {
        "info":
